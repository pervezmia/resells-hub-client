...
//                 isRequired
//                 name="images"
//                 defaultValue={getSafeImage(product.images?.join(", "))}
//                 validate={(value) => {
//                   const urls = value
//                     .split(",")
//                     .map((url) => url.trim())
...
//                   return null; // সব ঠিক থাকলে null রিটার্ন করতে হয়
//                 }}
//               >
...
export default function EditProductForm({ product }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const result = await updateProduct(product._id || product.id, payload);

      // ফ্লেক্সিবল চেকিং: success flag অথবা mongo DB result
      if (
        result?.success ||
          ...
          <Fieldset className="w-full">
            <Fieldset.Group className="gap-4">
              ...
              <TextField
                ...
                validate={(value) => {
                  if (!value) return "At least one image URL is required";
